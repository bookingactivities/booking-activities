...
// GENERAL
	...
	function bookacti_woocommerce_translation_array( $translation_array ) {
		...
		$translation_array[ 'expired_min' ]						= __( 'expired', BOOKACTI_PLUGIN_NAME );
		$translation_array[ 'expired' ]							= __( 'Expired', BOOKACTI_PLUGIN_NAME );
		$translation_array[ 'in_cart' ]							= __( 'In cart', BOOKACTI_PLUGIN_NAME );
		$translation_array[ 'days' ]							= __( 'days', BOOKACTI_PLUGIN_NAME );
		$translation_array[ 'day' ]								= _x( 'day', 'singular of days',BOOKACTI_PLUGIN_NAME );
		$translation_array[ 'error_remove_expired_cart_item' ]	= __( 'Error occurs while trying to remove expired cart item.', BOOKACTI_PLUGIN_NAME );
...
// LOOP PRODUCTS PAGE
	...
	function bookacti_add_activity_post_class( $classes, $class, $post_id ) {
		...
		if( $is_activity ) { $classes[] = 'activity'; }
...
//ADD TO CART (SINGLE PRODUCT PAGE)
	...
	function bookacti_add_booking_system_in_single_product_page() {
		...
		if( $is_activity ) {
			...
			$booking_method	= get_post_meta( $product->id, '_bookacti_booking_method', true );
			$template_id	= get_post_meta( $product->id, '_bookacti_template', true );
			$activity_id	= get_post_meta( $product->id, '_bookacti_activity', true );
			
			bookacti_display_booking_system( array( $template_id ), array( $activity_id ), $booking_method, $product->id, 'frontend-booking-system woo-product-booking-system' );
		}
	}
	...
	function bookacti_format_order_meta_data( $formatted_meta, $instance ) {
		...
		foreach( $formatted_meta as $key => $meta ) {
			...
			if( $meta[ 'key' ] === 'bookacti_event_start' || $meta[ 'key' ] === 'bookacti_event_end' ) {
				...
				$formatted_meta[ $key ][ 'value' ] = strftime( __( '%A, %B %d, %Y %I:%M %p', BOOKACTI_PLUGIN_NAME ), strtotime( $value ) );
				
			} else if( $meta[ 'key' ] === 'bookacti_state' ) {
				
				$formatted_meta[ $key ][ 'value' ] = bookacti_format_booking_state( $value );
				
			} else if( $meta[ 'key' ] === 'bookacti_refund_coupon' ) {
				if( ! class_exists( 'WC_API_Coupons' ) ) {
					// include & load API classes
					WC()->api->includes();
					WC()->api->register_resources( new WC_API_Server( '/' ) );
				}
				$coupon = WC()->api->WC_API_Coupons->get_coupon( $formatted_meta[ $key ][ 'value' ] );
				$formatted_meta[ $key ][ 'value' ] = $coupon['coupon']['code'];
			}
		}
	...
	function bookacti_set_timeout_to_cart_item( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
		...
		if( $is_activity ) {
			...
			//Keep the booking id in memory to retrieve it with cart item key later
			...
			if( $is_expiration_active ) {
				...
				//Set expiration date to the product that has been added to cart
				...
				$message = sprintf( __( 'The schedule is temporarily booked for %1$s. Please proceed to checkout.', BOOKACTI_PLUGIN_NAME ), $timeout_formated );

				//Tell the user that the cart will soon expire
	...
	// Do not display "In stock" for activities
	add_filter( 'woocommerce_stock_html', 'bookacti_dont_display_instock_in_variation', 10, 3 );
	function bookacti_dont_display_instock_in_variation( $availability_html, $availability, $variation ) {
