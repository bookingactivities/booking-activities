...
function bookacti_dialog_remove_form_field(a,b){var d=$j.trim($j("#bookacti-remove-form-field-dialog").dialog("option","title").replace(/\(.*?\)/,""));$j("#bookacti-remove-form-field-dialog").dialog({title:d+" ("+bookacti_localized.edit_id+": "+a+")"});$j("#bookacti-remove-form-field-dialog").dialog("option","buttons",[{text:bookacti_localized.dialog_button_ok,click:function(){var c=$j("#nonce_remove_form_field").val();a&&c&&(bookacti_form_editor_enter_loading_state(),$j.ajax({url:ajaxurl,type:"POST",
data:{action:"bookactiRemoveFormField",field_id:a,nonce:c},dataType:"json",success:function(f){"success"===f.status?($j("#bookacti-form-editor-field-"+a).remove(),delete bookacti.form_editor.fields[a],bookacti.form_editor.form.field_order=f.field_order,$j('#bookacti-field-to-insert option[value="'+b+'"]').attr("disabled",!1),$j("#bookacti-form-editor").trigger("bookacti_field_removed",[a,b])):"failed"===f.status&&(console.log("undefined"!==typeof f.message?f.message:bookacti_localized.error),console.log(f))},
error:function(f){console.log("AJAX "+bookacti_localized.error);console.log(f)},complete:function(){bookacti_form_editor_exit_loading_state()}}),$j(this).dialog("close"))}}]);$j("#bookacti-remove-form-field-dialog").dialog("open")}
...
b+' input[name="action"]').val("bookactiResetFormField");var c=$j("form#bookacti-form-field-form-"+b).serializeObject(),f=$j("#bookacti-form-editor-field-"+a+" .bookacti-form-editor-field-body").is(":visible");bookacti_form_editor_enter_loading_state();$j.ajax({url:ajaxurl,type:"POST",data:c,dataType:"json",success:function(e){"success"===e.status?("calendar"!==b&&($j("#bookacti-form-editor-field-"+a).replaceWith(e.field_html),f&&$j("#bookacti-form-editor-field-"+a+" .bookacti-form-editor-field-header").trigger("click")),
...
function bookacti_dialog_export_events(a){$j("#bookacti-export-events-dialog .bookacti-notices").remove();$j("#bookacti_export_events_url_secret").val($j("#bookacti_export_events_url_secret").data("value"));$j("#bookacti-export-events-dialog").dialog("option","buttons",[{text:bookacti_localized.dialog_button_ok,click:function(){$j("#bookacti-export-events-dialog .bookacti-notices").remove();$j(this).dialog("close")}},{text:bookacti_localized.dialog_button_reset,"class":"bookacti-dialog-delete-button bookacti-dialog-left-button",
click:function(){var b=$j("#nonce_reset_export_events_url").val();a&&b&&($j("#bookacti-export-events-dialog .bookacti-notices").remove(),bookacti_form_editor_enter_loading_state(),$j.ajax({url:ajaxurl,type:"POST",data:{action:"bookactiResetExportEventsUrl",form_id:a,nonce:b},dataType:"json",success:function(d){if("success"===d.status){var c=$j("#bookacti_export_events_url_secret").data("value").replace(d.old_secret_key,d.new_secret_key);$j("#bookacti_export_events_url_secret").data("value",c);$j("#bookacti_export_events_url_secret").attr("data-value",
c);$j("#bookacti_export_events_url_secret").val(c);$j("#bookacti-export-events-dialog").append('<div class="bookacti-notices"><ul class="bookacti-success-list"><li>'+d.message+"</li></ul></div>").show();$j("#bookacti-form-editor").trigger("bookacti_export_events_url_reset",[a,d])}else"failed"===d.status&&(c="undefined"!==typeof d.message?d.message:bookacti_localized.error,$j("#bookacti-export-events-dialog").append('<div class="bookacti-notices"><ul class="bookacti-error-list"><li>'+c+"</li></ul></div>").show(),
console.log(c),console.log(d))},error:function(d){$j("#bookacti-export-events-dialog").append('<div class="bookacti-notices"><ul class="bookacti-error-list"><li>AJAX '+bookacti_localized.error+"</li></ul></div>").show();console.log("AJAX "+bookacti_localized.error);console.log(d)},complete:function(){$j("#bookacti-export-events-dialog .bookacti-notices").show();bookacti_form_editor_exit_loading_state()}}))}}]);$j("#bookacti-export-events-dialog").dialog("open")}
