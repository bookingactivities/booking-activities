...
function bookacti_init_jquery_ui_dialogs(a){typeof a==="undefined"&&(a=".bookacti-backend-dialog");$j(a).dialog({modal:!0,autoOpen:!1,minHeight:300,minWidth:460,resize:"auto",show:!0,hide:!0,dialogClass:"bookacti-dialog",beforeClose:function(){if(bookacti_localized.is_admin&&typeof bookacti_empty_all_dialog_forms!=="undefined"){var b=$j(this).attr("id");b&&(a="#"+b);bookacti_empty_all_dialog_forms(a)}}})}
function bookacti_get_loading_html(){return'<div class="bookacti-loading-container"><div class="bookacti-loading-image"><div class="bookacti-spinner"></div></div><div class="bookacti-loading-text">'+bookacti_localized.loading+"</div></div>"}function bookacti_add_loading_html(a,b){b=b?b:"append";var d=bookacti_get_loading_html();b==="before"?a.before(d):b==="after"?a.after(d):b==="prepend"?a.prepend(d):a.append(d)}
function bookacti_remove_loading_html(a){a.find(".bookacti-loading-container").addBack(".bookacti-loading-container").remove()}function bookacti_scroll_to(a,b,d){b=$j.isNumeric(b)?parseInt(b):500;d=d!=="middle"?"top":"middle";var c=typeof a==="number"?a:a.length?a.offset().top:$j(document).scrollTop(),e=c;d==="middle"&&typeof a!=="number"&&a.length&&(a=a.height(),d=$j(window).height(),a<d&&(e=c-(d/2-a/2)));$j("html, body").animate({scrollTop:e},b)}
function bookacti_pad(a,b){a=a.toString();return a.length<b?bookacti_pad("0"+a,b):a}function bookacti_is_numeric(a){var b=jQuery.type(a);return(b==="number"||b==="string")&&!isNaN(a-parseFloat(a))}function bookacti_compare_arrays(a,b){return $j(a).not(b).length===0&&$j(b).not(a).length===0}
function bookacti_serialize_object(a){function b(c,e){if(c.length<1)return e;var f=c[0];f.slice(-1)=="]"&&(f=f.slice(0,-1));var g={};c.length==1?g[f]=e:(c.shift(),c=b(c,e),g[f]=c);return g}var d={};a=function(c){var e=[];$j.each(c.serializeArray(),function(f,g){if(g.name.indexOf("[]")<0)return e.push(g),!0;var h=g.name.split("[]")[0],k=!1;$j.each(e,function(l,m){m.name===h&&(k=!0,e[l].value.push(g.value))});k||e.push({name:h,value:[g.value]})});return e}(a);$j.each(a,function(){var c=this.value,e=
...
text:$j(this).text()})});b=$j.extend(c,{action:d?"bookactiSelect2Query_"+d:"bookactiSelect2Query",term:typeof b.term=="string"?b.term:"",options:e,name:$j(this).attr("name")?$j(this).attr("name"):"",id:$j(this).attr("id")?$j(this).attr("id"):"",nonce:bookacti_localized.nonce_query_select2_options});$j(this).trigger("bookacti_select2_query_data",[b]);return b},processResults:function(b){var d={results:typeof b.options!=="undefined"?b.options:[]};$j(this).trigger("bookacti_select2_query_results",[d,
...
f){e=!1;typeof $j(f).data("data")!=="undefined"&&typeof $j(f).data("data").id!=="undefined"&&(e=$j(f).data("data").id);if(e===!1)return!0;f=c.find('option[value="'+e+'"]');if(!f.length)return!0;f.detach();c.append(f)})}})}function bookacti_select2_destroy(a){$j.fn.select2&&$j(a).length&&$j(a).hasClass("select2-hidden-accessible")&&$j(a).select2("destroy")}
...
function bookacti_number_format(a,b,d,c){a=parseFloat(a);formatted_number=a.toFixed(parseInt(b));parseFloat(formatted_number).toString().indexOf(".")===-1&&(formatted_number=parseFloat(formatted_number));a=formatted_number.toString().split(".");a[0]=a[0].replace(/\B(?=(\d{3})+(?!\d))/g,c);return formatted_number=a.join(d)}
function bookacti_get_string_between(a,b,d){typeof a==="string"||a instanceof String||(a="");typeof b==="string"||b instanceof String||(b="");typeof d==="string"||d instanceof String||(d="");var c=b!==""?a.indexOf(b):-1,e=!1;c<0&&(c=0,e=!0);c+=b.length;len=d!==""&&c<=a.length?a.indexOf(d,c):-1;var f=!1;len<0?(len=a.length,f=!0):len-=c;return b!==""&&e||d!==""&&f?"":a.substring(c,len)};
