...
$j(this).hasClass("bookacti-delete-booking")&&bookacti_dialog_delete_booking(b,"single")}else b=$j(this).data("booking-group-id"),a.booking_group_ids.push(b),$j(this).hasClass("bookacti-cancel-booking-group")?bookacti_dialog_cancel_booking(b,"group"):$j(this).hasClass("bookacti-refund-booking-group")?bookacti_dialog_refund_booking(b,"group"):$j(this).hasClass("bookacti-change-booking-group-state")?bookacti_dialog_change_bookings_status(a):$j(this).hasClass("bookacti-change-booking-group-quantity")?
bookacti_dialog_change_bookings_quantity(a):$j(this).hasClass("bookacti-show-booking-group-bookings")?bookacti_display_grouped_bookings(b):$j(this).hasClass("bookacti-send-booking-group-notification")?bookacti_dialog_send_bookings_notification(a):$j(this).hasClass("bookacti-delete-booking-group")&&bookacti_dialog_delete_booking(b,"group");$j(this).attr("href")&&""!==$j(this).attr("href")&&!$j(this).hasClass("prevent-default")&&($j(this).hasClass("_blank")?window.open($j(this).attr("href")):location.href=
$j(this).attr("href"))})}
function bookacti_init_booking_bulk_actions(){$j("#bookacti-bookings-container").on("click",'.bulkactions input[type="submit"]',function(a){a.preventDefault();if(a=$j(this).siblings('select[name^="action"]').val()){var b={booking_ids:[],booking_group_ids:[],all:parseInt($j("#bookacti-all-selected").val())?1:0};$j('#bookacti-bookings-container tbody .check-column input[name="booking_ids[]"]:checked').each(function(){b.booking_ids.push(parseInt($j(this).val()))});$j('#bookacti-bookings-container tbody .check-column input[name="booking_group_ids[]"]:checked').each(function(){b.booking_group_ids.push(parseInt($j(this).val()))});"reschedule"===
a?bookacti_dialog_reschedule_booking(booking_id):"refund"===a?bookacti_dialog_refund_booking(booking_id,"single"):"edit_status"===a?bookacti_dialog_change_bookings_status(b):"edit_quantity"===a?bookacti_dialog_change_bookings_quantity(b):"send_notification"===a?bookacti_dialog_send_bookings_notification(b):"delete"===a&&bookacti_dialog_delete_booking(booking_id,"single")}})}
