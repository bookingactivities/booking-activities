...
"tbody .toggle-row",function(){$j(this).closest("tr").toggleClass("is-expanded")});$j("#bookacti-booking-list tbody").off("click",".toggle-row")});
...
function bookacti_init_booking_actions(){$j(".bookacti-user-booking-list-table, .woocommerce-table, #bookacti-booking-list").on("click",".bookacti-booking-action, .bookacti-booking-group-action",function(a){a.preventDefault();bookacti.user_auth_key=$j(this).data("user-auth-key")?$j(this).data("user-auth-key"):"";a={booking_ids:[],booking_group_ids:[],all:0,filters:{}};if($j(this).hasClass("bookacti-booking-action")){var b=$j(this).data("booking-id");b&&a.booking_ids.push(parseInt(b))}else(b=$j(this).data("booking-group-id"))&&
a.booking_group_ids.push(parseInt(b));if(a.booking_ids.length||a.booking_group_ids.length)$j(this).hasClass("bookacti-cancel-booking")?bookacti_dialog_cancel_bookings(a):$j(this).hasClass("bookacti-reschedule-booking")?bookacti_dialog_reschedule_bookings(a):$j(this).hasClass("bookacti-refund-booking")?bookacti_dialog_refund_bookings(a):$j(this).hasClass("bookacti-change-booking-status")?bookacti_dialog_change_bookings_status(a):$j(this).hasClass("bookacti-change-booking-quantity")?bookacti_dialog_change_bookings_quantity(a):
$j(this).hasClass("bookacti-send-booking-notification")?bookacti_dialog_send_bookings_notification(a):$j(this).hasClass("bookacti-delete-booking")?bookacti_dialog_delete_bookings(a):$j(this).hasClass("bookacti-show-booking-group-bookings")&&bookacti_display_grouped_bookings(a);$j(this).attr("href")&&""!==$j(this).attr("href")&&!$j(this).hasClass("prevent-default")&&($j(this).hasClass("_blank")?window.open($j(this).attr("href")):location.href=$j(this).attr("href"))})}
...
$j(this).val();e&&b.booking_ids.push(parseInt(e))});$j('#bookacti-bookings-container tbody .check-column input[name="booking_group_ids[]"]:checked').each(function(){var e=$j(this).val();e&&b.booking_group_ids.push(parseInt(e))});if(b.booking_ids.length||b.booking_group_ids.length||b.all&&!$j.isEmptyObject(b.filters))"reschedule"===a?bookacti_dialog_reschedule_bookings(b):"refund"===a?bookacti_dialog_refund_bookings(b):"edit_status"===a?bookacti_dialog_change_bookings_status(b):"edit_quantity"===a?
bookacti_dialog_change_bookings_quantity(b):"send_notification"===a?bookacti_dialog_send_bookings_notification(b):"delete"===a?bookacti_dialog_delete_bookings(b):"display_grouped_bookings"===a?bookacti_display_grouped_bookings(b):"cancel"===a&&bookacti_dialog_cancel_bookings(b)}})}
...
d}),b=b.not($j(this)))});console.log("rows.length",b.length);console.log("booking_selection.booking_group_ids",a.booking_group_ids);if(b.length&&a.booking_group_ids.length){var e=[];b.first().find("td").each(function(){var d=$j(this).data("column-id");d&&e.push(d)});a.all=!1;a.filters={};var c={form_data:new FormData};c.form_data.append("action","bookactiGetGroupedBookingsRows");c.form_data.append("booking_selection",JSON.stringify(a));c.form_data.append("columns",JSON.stringify(e));c.form_data.append("is_admin",
bookacti_localized.is_admin?1:0);c.form_data.append("user_auth_key","undefined"!==typeof bookacti.user_auth_key?bookacti.user_auth_key:"");c.form_data.append("nonce",bookacti_localized.nonce_get_booking_rows);$j("body").trigger("bookacti_booking_action_data",[c,a,"display_grouped_bookings"]);bookacti_booking_row_enter_loading_state(b);$j.ajax({url:bookacti_localized.ajaxurl,type:"POST",data:c.form_data,dataType:"json",cache:!1,contentType:!1,processData:!1,success:function(d){if("success"===d.status){$j("#bookacti-booking-list-container #the-list tr.no-items").remove();
