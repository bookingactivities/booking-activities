...
function bookacti_init_booking_actions(){$j(".bookacti-user-booking-list-table, .woocommerce-table, #bookacti-booking-list").on("click",".bookacti-booking-action, .bookacti-booking-group-action",function(a){a.preventDefault();user_auth_key=$j(this).data("user-auth-key").length?$j(this).data("user-auth-key"):"";$j(this).hasClass("bookacti-booking-action")?(a=$j(this).data("booking-id"),$j(this).hasClass("bookacti-cancel-booking")?bookacti_dialog_cancel_booking(a,"single"):$j(this).hasClass("bookacti-reschedule-booking")?
bookacti_dialog_reschedule_booking(a):$j(this).hasClass("bookacti-refund-booking")?bookacti_dialog_refund_booking(a,"single"):$j(this).hasClass("bookacti-change-booking-status")?bookacti_dialog_change_booking_state(a,"single"):$j(this).hasClass("bookacti-change-booking-quantity")?bookacti_dialog_change_booking_quantity(a,"single"):$j(this).hasClass("bookacti-send-booking-notification")?bookacti_dialog_send_booking_notification(a,"single"):$j(this).hasClass("bookacti-delete-booking")&&bookacti_dialog_delete_booking(a,
"single")):(a=$j(this).data("booking-group-id"),$j(this).hasClass("bookacti-cancel-booking-group")?bookacti_dialog_cancel_booking(a,"group"):$j(this).hasClass("bookacti-refund-booking-group")?bookacti_dialog_refund_booking(a,"group"):$j(this).hasClass("bookacti-change-booking-group-state")?bookacti_dialog_change_booking_state(a,"group"):$j(this).hasClass("bookacti-change-booking-group-quantity")?bookacti_dialog_change_booking_quantity(a,"group"):$j(this).hasClass("bookacti-show-booking-group-bookings")?
bookacti_display_grouped_bookings(a):$j(this).hasClass("bookacti-send-booking-group-notification")?bookacti_dialog_send_booking_notification(a,"group"):$j(this).hasClass("bookacti-delete-booking-group")&&bookacti_dialog_delete_booking(a,"group"));$j(this).attr("href")&&""!==$j(this).attr("href")&&!$j(this).hasClass("prevent-default")&&($j(this).hasClass("_blank")?window.open($j(this).attr("href")):location.href=$j(this).attr("href"))})}
function bookacti_init_booking_bulk_actions(){$j("#bookacti-bookings-container").on("click",'.bulkactions input[type="submit"]',function(a){a.preventDefault();if(a=$j(this).siblings('select[name^="action"]').val()){var b={booking_ids:[],booking_group_ids:[],all:parseInt($j("#bookacti-all-selected").val())?1:0};b.all||($j('#bookacti-bookings-container tbody .check-column input[name="booking_ids[]"]:checked').each(function(){b.booking_ids.push(parseInt($j(this).val()))}),$j('#bookacti-bookings-container tbody .check-column input[name="booking_group_ids[]"]:checked').each(function(){b.booking_group_ids.push(parseInt($j(this).val()))}));
"reschedule"===a?bookacti_dialog_reschedule_booking(booking_id):"refund"===a?bookacti_dialog_refund_booking(booking_id,"single"):"edit_status"===a?bookacti_dialog_change_bookings_status(b):"edit_quantity"===a?bookacti_dialog_change_booking_quantity(booking_id,"single"):"send_notification"===a?bookacti_dialog_send_booking_notification(booking_id,"single"):"delete"===a&&bookacti_dialog_delete_booking(booking_id,"single")}})}
