...
bookacti_add_loading_html($j("#bookacti-delete-template-dialog"));$j.ajax({url:ajaxurl,data:c,type:"POST",dataType:"json",success:function(b){if("success"===b.status){$j("#bookacti-template-picker option[value="+a+"]").remove();$j("select.bookacti-template-selectbox option[value="+a+"]").remove();var d=$j("#bookacti-template-picker option:first").val();1===$j("#bookacti-template-picker option").length&&bookacti_bind_template_dialogs();$j("#bookacti-delete-template-dialog").trigger("bookacti_template_deactivated",
...
$j("#bookacti-activity-data-dialog").trigger("bookacti_activity_insert_dialog");$j("#bookacti-activity-data-dialog").dialog("option","buttons",[{text:bookacti_localized.dialog_button_ok,click:function(){$j("#bookacti-activity-data-dialog .bookacti-notices").remove();"undefined"!==typeof tinyMCE&&tinyMCE&&tinyMCE.triggerSave();if(bookacti_validate_activity_form()){var c=bookacti_serialize_object($j("#bookacti-activity-data-form"));$j("#bookacti-activity-data-dialog").trigger("bookacti_insert_activity_before",
[c]);bookacti_start_template_loading();bookacti_add_loading_html($j("#bookacti-activity-data-dialog"));$j.ajax({url:ajaxurl,data:c,type:"POST",dataType:"json",success:function(b){if("success"===b.status)bookacti.booking_system["bookacti-template-calendar"].activities_data[b.activity_id]=b.activity_data,b.activity_list&&$j("#bookacti-template-activity-list").empty().append(b.activity_list),bookacti_refresh_activity_list(),$j("select.bookacti-activity-selectbox").append('<option value="'+b.activity_id+
'">'+b.activity_data.title+"</option>"),$j("#bookacti-activity-data-dialog").trigger("bookacti_activity_inserted",[b,c]),$j("#bookacti-activity-data-dialog").dialog("close"),$j("#bookacti-activity-create-method-dialog").dialog("close");else if("failed"===b.status){var d="undefined"!==typeof b.message?b.message:bookacti_localized.error;$j("#bookacti-activity-data-dialog").append('<div class="bookacti-notices"><ul class="bookacti-error-list"><li>'+d+"</li></ul></div>");$j("#bookacti-activity-data-dialog .bookacti-notices").show();
