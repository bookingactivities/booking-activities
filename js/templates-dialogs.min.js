...
"buttons",[{text:bookacti_localized.dialog_button_ok,click:function(){$j("#bookacti-template-data-dialog .bookacti-notices").remove();$j("#bookacti-template-data-form-action").val("bookactiInsertTemplate");var c=$j("#bookacti-template-title").val();"undefined"!==typeof tinyMCE&&tinyMCE&&tinyMCE.triggerSave();if(bookacti_validate_template_form()){var b=$j("#bookacti-template-data-form").serializeObject();$j("#bookacti-template-data-dialog").trigger("bookacti_insert_template_before",[b]);$j("#bookacti-template-calendar").length?
bookacti_start_template_loading():$j("#bookacti-add-first-template-button").length&&$j("#bookacti-add-first-template-button").removeClass("dashicons dashicons-plus-alt").addClass("spinner");bookacti_add_loading_html($j("#bookacti-template-data-dialog"));$j.ajax({url:ajaxurl,data:b,type:"POST",dataType:"json",success:function(d){if("success"===d.status){var e=!1;$j("#bookacti-first-template-container").is(":visible")&&($j("#bookacti-first-template-container").before($j('<div class="bookacti-booking-system-editor" id="bookacti-template-calendar"><div class="bookacti-calendar"></div>')),
...
$j("#bookacti-activity-data-dialog").dialog("option","buttons",[{text:bookacti_localized.dialog_button_ok,click:function(){$j("#bookacti-activity-data-dialog .bookacti-notices").remove();"undefined"!==typeof tinyMCE&&tinyMCE&&tinyMCE.triggerSave();if(bookacti_validate_activity_form()){var c=$j("#bookacti-activity-data-form").serializeObject();$j("#bookacti-activity-data-dialog").trigger("bookacti_insert_activity_before",[c]);bookacti_start_template_loading();bookacti_add_loading_html($j("#bookacti-activity-data-dialog"));
...
$j("#bookacti-group-of-events-action").val("bookactiInsertGroupOfEvents");var b=$j("#bookacti-group-of-events-category-selectbox").val();bookacti.selected_category=b;"undefined"!==typeof tinyMCE&&tinyMCE&&tinyMCE.triggerSave();if(bookacti_validate_group_of_events_form()){var d=$j("#bookacti-group-of-events-form").serializeObject();d.action="bookactiInsertGroupOfEvents";d.template_id=bookacti.selected_template;d.interval=JSON.stringify(bookacti.booking_system["bookacti-template-calendar"].events_interval);
d.events=JSON.stringify(bookacti.booking_system["bookacti-template-calendar"].selected_events);$j("#bookacti-group-of-events-dialog").trigger("bookacti_insert_group_of_events_before",[d,a]);bookacti_start_template_loading();bookacti_add_loading_html($j("#bookacti-group-of-events-dialog"));$j.ajax({url:ajaxurl,data:d,type:"POST",dataType:"json",success:function(e){if("success"===e.status)bookacti.booking_system["bookacti-template-calendar"].groups_data[e.group_id]=e.group_data,bookacti.booking_system["bookacti-template-calendar"].groups_events[e.group_id]=
e.group_events,bookacti.booking_system["bookacti-template-calendar"].group_categories_data[e.category_id]=e.category,bookacti.booking_system["bookacti-template-calendar"].selected_events=[],$j("#bookacti-template-add-first-group-of-events-container").hide(),"new"===b&&bookacti_add_group_category(e.category_id,e.category.title),bookacti_add_group_of_events(e.group_id,e.group_title_raw,e.category_id),bookacti_unselect_all_events(),bookacti.fc_calendar["bookacti-template-calendar"].render(),$j("#bookacti-group-of-events-dialog").trigger("bookacti_group_of_events_inserted",
[e,d,a]),$j("#bookacti-group-of-events-dialog").dialog("close");else{var f="undefined"!==typeof e.message?e.message:bookacti_localized.error;$j("#bookacti-group-of-events-dialog").append('<div class="bookacti-notices"><ul class="bookacti-error-list"><li>'+f+"</li></ul></div>");$j("#bookacti-group-of-events-dialog .bookacti-notices").show();console.log(e)}},error:function(e){alert("AJAX "+bookacti_localized.error);console.log(e)},complete:function(){bookacti_stop_template_loading();bookacti_remove_loading_html($j("#bookacti-group-of-events-dialog"))}})}}}]);
...
bookacti.fc_calendar["bookacti-template-calendar"].gotoDate(d[0].start);var f={action:"bookactiBeforeDeleteGroupOfEvents",group_id:a,nonce:$j("#nonce_delete_group_of_events").val()};$j("#bookacti-group-of-events-dialog").trigger("bookacti_before_deactivate_group_of_events",[f,a]);bookacti_start_template_loading();bookacti_add_loading_html($j("#bookacti-group-of-events-dialog"));$j.ajax({url:ajaxurl,data:f,type:"POST",dataType:"json",success:function(g){if("success"===g.status)$j("#bookacti-group-of-events-dialog").dialog("close"),
