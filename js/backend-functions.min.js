...
if(!f){var e=null;c&&($j.isArray(c)||(c=[c]),$j.each(c,function(g,h){g=$j("select#"+b+' option[value="'+h+'"]');if(g.length&&!g.is(":disabled"))return e=g.attr("value"),!1}));$j("select#"+b).val(e).trigger("change")}if($j("select#"+b).hasClass("select2-hidden-accessible")){if(!$j.fn.select2)return;$j("select#"+b).select2("destroy");bookacti_select2_init()}$j("select#"+b).trigger("bookacti_switched_multiple",[f])}}
