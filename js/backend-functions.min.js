...
function bookacti_empty_all_dialog_forms(a){a="undefined"!==typeof a&&a?a+" ":".bookacti-backend-dialog ";$j(a+".bookacti-form-error").remove();$j(a+".bookacti-notices").remove();$j(a+".bookacti-input-warning").removeClass("bookacti-input-warning");$j(a+".bookacti-input-error").removeClass("bookacti-input-error");$j(a+'input[type="hidden"]:not([name="action"]):not([name^="nonce"]):not([name="_wp_http_referer"]):not([name="qtranslate-edit-language"]):not(.bookacti-onoffswitch-hidden-input)').val("");$j(a+
'input[type="text"]:not([readonly])').val("");$j(a+'input[type="email"]').val("");$j(a+'input[type="password"]').val("");$j(a+'input[type="tel"]').val("");$j(a+'input[type="number"]').val("");$j(a+'input[type="date"]').val("");$j(a+'input[type="time"]').val("");$j(a+"textarea").val("");$j(a+'input[type="color"]').val("#3a87ad");$j(a+'input[type="checkbox"]').prop("checked",!1);$j(a+'input[type="radio"]').prop("checked",!1);$j(a+"select").val(null).trigger("change");$j(a+".bookacti-duration-hint").remove();
...
!1);f&&0>d.indexOf("[]")?($j("select#"+b).attr("name",d+"[]"),$j(a).closest(".bookacti-multiple-select-container").find("label span.dashicons").removeClass("dashicons-plus").addClass("dashicons-minus")):($j("select#"+b).attr("name",d.replace("[]","")),$j(a).closest(".bookacti-multiple-select-container").find("label span.dashicons").removeClass("dashicons-minus").addClass("dashicons-plus"));if(!f){var e=null;c&&($j.isArray(c)||(c=[c]),$j.each(c,function(g,h){g=$j("select#"+b+' option[value="'+h+'"]');
if(g.length&&!g.is(":disabled"))return e=g.attr("value"),!1}));$j("select#"+b).val(e).trigger("change")}if($j("select#"+b).hasClass("select2-hidden-accessible")){if(!$j.fn.select2)return;$j("select#"+b).select2("destroy");bookacti_select2_init()}$j("select#"+b).trigger("bookacti_switched_multiple")}}
