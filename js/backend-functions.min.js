...
function bookacti_switch_select_to_multiple(a){if($j(a).length){var b=$j(a).data("select-id"),d=$j("select#"+b).attr("name"),f=$j(a).is(":checked"),c=$j("select#"+b).val();$j("select#"+b).prop("multiple",f);$j("select#"+b).trigger("bookacti_switch_multiple_before",[f]);$j("select#"+b+' option[value="all"]').prop("disabled",f);$j("select#"+b+' option[value="none"]').prop("disabled",f);$j("select#"+b+' option[value="parent"]').prop("disabled",f);$j("select#"+b+' option[value="site"]').prop("disabled",
f);$j("select#"+b+' option[data-not-multiple="1"]').prop("disabled",f);$j("select#"+b+" option:disabled:selected").prop("selected",!1);f&&0>d.indexOf("[]")?($j("select#"+b).attr("name",d+"[]"),$j(a).closest(".bookacti-multiple-select-container").find("label span.dashicons").removeClass("dashicons-plus").addClass("dashicons-minus")):($j("select#"+b).attr("name",d.replace("[]","")),$j(a).closest(".bookacti-multiple-select-container").find("label span.dashicons").removeClass("dashicons-minus").addClass("dashicons-plus"));
if(!f){var e=null;c&&($j.isArray(c)||(c=[c]),$j.each(c,function(g,h){g=$j("select#"+b+' option[value="'+h+'"]');if(g.length&&!g.is(":disabled"))return e=g.attr("value"),!1}));$j("select#"+b).val(e).trigger("change")}$j("select#"+b).hasClass("select2-hidden-accessible")&&(bookacti_select2_destroy($j("select#"+b)),bookacti_select2_init());$j("select#"+b).trigger("bookacti_switched_multiple",[f])}}
