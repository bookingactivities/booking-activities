...
$j("#bookacti-event-booking-list-columns-container").hide()});$j("#bookacti-booking-list-filters-form").on("submit",function(a){$j("#bookacti-submit-filter-button").data("ajax")&&(a.preventDefault(),bookacti_filter_booking_list())});bookacti_refresh_booking_group_frame();$j("#bookacti-bookings-container").on("change","#current-page-selector",function(a){if($j("#bookacti-submit-filter-button").data("ajax")){"undefined"!==typeof bookacti_booking_list_paged_monitor&&bookacti_booking_list_paged_monitor&&
clearTimeout(bookacti_booking_list_paged_monitor);var b=$j(this).val();bookacti_booking_list_paged_monitor=setTimeout(function(){bookacti_filter_booking_list(b)},750)}});$j("#bookacti-bookings-container").on("click",".first-page, .prev-page, .next-page, .last-page",function(a){if($j("#bookacti-submit-filter-button").data("ajax")){a.preventDefault();a=$j(this).attr("href");var b=a.indexOf("paged=");a=-1!==b?a.substr(b+6):1;bookacti_filter_booking_list(a)}});$j("body").on("click",".bookacti-list-table .check-column input",
function(){var a=$j("#bookacti-bookings-container tbody .check-column input:checked").length;if(a){$j("#bookacti-bookings-container .bookacti-select-all-container").remove();$j("#bookacti-all-selected").val(0);a='<span class="bookacti-nb-selected">'+bookacti_localized.nb_selected.replace("{nb}",a)+"</span>";var b='<button class="bookacti-select-all button">'+bookacti_localized.select_all.replace("{nb}",$j("#bookacti-bookings-container .displaying-num").first().text())+"</button>";$j("#bookacti-bookings-container .tablenav .bulkactions").append('<span class="bookacti-select-all-container">'+
