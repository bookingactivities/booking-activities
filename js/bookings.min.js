...
c,b){a=-1;for(var d=0;d<c.domNodes.length;d++)if(c.domNodes[d].classList.contains("bookacti-availability-container")){a=d;break}0>a||(b=parseInt(bookacti.booking_system[f].events_data[b.event.groupId].availability),d=$j(c.domNodes[a]),d.find(".bookacti-available-places").append(' / <span class="bookacti-total-places-number">'+b+"</span>"),c.domNodes[a]=d[0])});$j("#bookacti-bookings-calendar-settings").on("click",function(){bookacti_dialog_update_bookings_calendar_settings()});$j("#bookacti-bookings-calendar-settings-dialog").on("change",
