...
function bookacti_fc_get_events_by_groupId(a,d){a=a.attr("id");return $j.grep(bookacti.fc_calendar[a].getEvents(),function(b){return b.groupId==d})}function bookacti_fc_add_events(a,d){if(d.length){var b=[];$j.each(d,function(h,k){h=$j.extend(!0,{},k);h.groupId=parseInt(k.id);h.id=k.id+"_"+k.start;b.push(h)});d=a.attr("id");var f={events:b,editable:"bookacti-template-calendar"===d};a.trigger("bookacti_fc_events",[f]);bookacti.fc_calendar[d].addEventSource(f)}}
