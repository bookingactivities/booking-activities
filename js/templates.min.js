...
activity_id:c,template_id:bookacti.selected_template,title:b.multilingual_title,start:n,end:d,availability:b.availability,nonce:$j("#nonce_edit_template").val()};f.trigger("bookacti_calendar_editor_event_before_insert",[a,p]);bookacti_start_template_loading();$j.ajax({url:ajaxurl,data:p,type:"POST",dataType:"json",success:function(k){if("success"===k.status)a.event.setProp("groupId",k.event_id),$j('.fc-event[data-event-id=""][data-activity-id="'+c+'"][data-event-start="'+n+'"]').data("event-id",k.event_id).attr("data-event-id",
k.event_id),bookacti.booking_system["bookacti-template-calendar"].events_data[k.event_id]=k.event_data,f.trigger("bookacti_calendar_editor_event_inserted",[a,k,p]);else{a.revert();var q="undefined"!==typeof k.message?k.message:bookacti_localized.error;alert(q);console.log(q);console.log(k)}},error:function(k){a.revert();alert("AJAX "+bookacti_localized.error);console.log(k)},complete:function(){bookacti_stop_template_loading()}})}else a.revert()}},eventResize:function(a){bookacti_update_event_dates(a.event,
