...
1:0,p=$j("#bookacti-change-booking-state-dialog #nonce_change_booking_state").val();if((m||g)&&(m!==e||g!==f)){$j("#bookacti-change-booking-state-dialog .bookacti-notices").remove();var q=[];c.first().find("td").each(function(){var h=$j(this).data("column-id");h&&q.push(h)});n={action:d,booking_id:b,new_booking_state:e!==m?m:0,new_payment_status:g,send_notifications:n,is_bookings_page:k,is_admin:bookacti_localized.is_admin?1:0,context:bookacti_localized.is_admin?"admin_booking_list":"user_booking_list",
columns:q,reload_grouped_bookings:l,nonce:p};c.first().trigger("bookacti_booking_action_data",[n,b,a,"change_state"]);bookacti_booking_row_enter_loading_state(c);bookacti_add_loading_html($j("#bookacti-change-booking-state-dialog"));$j.ajax({url:bookacti_localized.ajaxurl,type:"POST",data:n,dataType:"json",success:function(h){if("success"===h.status)$j("#bookacti-change-booking-state-dialog").dialog("close"),l&&"undefined"!==typeof h.grouped_booking_rows&&c.each(function(){var t=$j(this).next().hasClass("hidden");
