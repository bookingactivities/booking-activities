...
$action             = ! empty( $_GET[ 'action' ] ) && in_array( $_GET[ 'action' ], array( 'edit', 'new' ), true ) ? $_GET[ 'action' ] : '';
...
if( ! $action ) { return; }
if( $action !== 'new' && ! $notification_db_id && ! $notification_id ) { return; }
...
if( $action !== 'new' && ! $notification_raw ) { return; }
...
$can_edit_notification   = $action === 'new' ? current_user_can( 'bookacti_create_notifications' ) : current_user_can( 'bookacti_edit_notifications' );
$can_manage_notification = $action === 'new' ? $can_edit_notification : bookacti_user_can_manage_notification( $notification_db_id );
if( ! $can_edit_notification || ! $can_manage_notification ) { esc_html_e( 'You are not allowed to do that.', 'booking-activities' ); return; }

?>
<div class='wrap'>
	<h1><?php echo ! $notification_db_id ? esc_html__( 'Create Notification', 'booking-activities' ) : /* translators: %s is the notification user-friendly identifier */ sprintf( esc_html__( 'Edit Notification - %s', 'booking-activities' ), '<code>' . $notification_id . '</code>' ); ?></h1>
	...
	<div id='bookacti-notification-editor-page-container'>
		<?php
			...
			$redirect_url = $notification_id ? 'admin.php?page=bookacti_notifications&action=edit&notification_id=' . $notification_id : 'admin.php?page=bookacti_notifications';
		?>
		<form name='post' action='<?php echo $redirect_url; ?>' method='post' id='bookacti-notification-editor-page-form' novalidate>
			<?php
			...
			<input type='hidden' name='action' value='<?php echo $notification_db_id ? 'edit' : 'create'; ?>'/>
			<input type='hidden' name='nonce' value='<?php echo wp_create_nonce( 'bookacti_update_notification' ); ?>'/>
			<input type='hidden' name='notification_db_id' value='<?php echo $notification_db_id; ?>' id='bookacti-notification-db-id'/>
					...
					<div id='post-body-content'>
						<div id='titlediv'>
							<div id='titlewrap'>
								...
								<input type='text' name='title' size='30' value='<?php echo esc_attr( $notification_edit[ 'title' ] ); ?>' id='title' class='bookacti-translatable' spellcheck='true' autocomplete='off' placeholder='<?php echo $title_placeholder; ?>' required/>
							</div>
						</div>
