 ...
 * @return string
 */
function bookacti_convert_events_to_ical( $events, $form_id = 0 ) {
	...
	/* translators: %1$s is a link to Booking Activities website. %2$s is the site URL. %3$s is the form name. */
	$description	= sprintf( esc_html__( 'This calendar was generated by %1$s from %2$s, from the form "%3$s".' ), 'Booking Activities (https://booking-activities.fr)', $site_name . ' (' . $site_url . ')', $form_name . ' (' . $form_id . ')' );
	...
	<?php
		...
		?>
			...
			<?php
				...
				do_action( 'bookacti_ical_vevent_after', $event, $events, $form_id );
			?>
			END:VEVENT
		<?php
		}

		do_action( 'bookacti_ical_vcalendar_after', $events, $form_id );
